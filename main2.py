...
#
# Makes this file work no matter which Python runs it — the
# setup.sh venv (/opt/pi-music-visualizer/venv/bin/python3) or a
# bare `python3 visualiser.py` on system Python that hasn't had
# `pip install pygame numpy sounddevice` run against it yet.
# ============================================================
_REQUIRED = ["numpy", "pygame", "sounddevice"]


def _pip_install(packages):
    for extra_args in (["--break-system-packages"], []):
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", *extra_args, *packages]
        )
...
def _bootstrap():
    ...
    if missing:
        print(
            f"[visualiser] Installing missing packages into {sys.executable}: "
            f"{', '.join(missing)}",
            file=sys.stderr,
        )
        if not _pip_install(missing):
            sys.exit(
                "[visualiser] Auto-install failed. Install manually:\n"
    ...
    try:
        ...
        # numpy/pygame import cleanly even when a system lib is missing;
        # sounddevice is the one that needs PortAudio installed at the OS
        # level (pip can't provide this on Raspberry Pi's arm64 wheels).
        sys.exit(
            f"[visualiser] {e}\n"
            "This is a missing SYSTEM library, not a Python package — pip can't fix it.\n"
